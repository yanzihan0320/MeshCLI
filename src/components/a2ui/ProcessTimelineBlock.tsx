...
export function ProcessTimelineBlock({ block }: { block: ProcessTimelineBlockData }) {
  return (
    <section className="rounded-2xl border border-border/70 bg-surface-900 p-3.5">
      <h3 className="mb-3 text-xs font-semibold tracking-[-0.01em] text-text-primary">{block.title}</h3>
        ...
        {block.steps.map((step, index) => {
          ...
          return (
            <li key={step.id} className="relative min-w-0 text-center">
              ...
              <div className="mt-1.5 truncate text-[10px] font-medium text-text-secondary">{step.label}</div>
              {step.detail && <div className="mt-0.5 text-[9px] leading-tight text-text-muted">{step.detail}</div>}
            </li>
          );
        })}
      </ol>
